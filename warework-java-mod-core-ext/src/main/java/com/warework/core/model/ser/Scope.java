...
public class Scope extends com.warework.core.model.Scope implements Serializable {
	...
	// Serial Version.
	private static final long serialVersionUID = 8114594592758917874L;
	...
	private Map<String, com.warework.core.model.Provider> providers;

	private Map<String, com.warework.core.model.Service> services;

	private Map<String, com.warework.core.model.ObjectReference> objectReferences;
	 ...
	 * 
	 * @param scopeName Name for the Scope. This name must be unique in order to
	 *                  create a new instance of a Scope, otherwise the factory will
	 *                  return an existing instance of a Scope. It cannot be
	 *                  <code>null</code> or represent an empty string.<br>
	 *                  <br>
	 */
	...
	/**
	 * Sets the Parent Scope of this Scope. Services, Providers and Object
	 * References that exist at Parent Scope are accesible from this Scope.
	 * 
	 * @param parent Parent Scope.<br>
	 *               <br>
	 */
	 ...
	 *              <br>
	 */
	public void setContextScope(final com.warework.core.model.Scope scope) {

		// Create the Scope's Context when required.
		if (context == null) {
			context = new HashMap<String, com.warework.core.model.ser.Scope>();
		}

		// Create a serializable domain Scope.
		final Scope serializableScope = new Scope();

		// Copy content of domain Scope.
		serializableScope.update(scope);

		// Set the parameter.
		context.put(scope.getName(), serializableScope);
	...
	public Enumeration<String> getContextScopeNames() {
		...
		if ((context != null) && (context.size() > 0)) {
			return DataStructureL2Helper.toEnumeration(context.keySet());
		}
	...
	public Enumeration<String> getInitParameterNames() {
		...
		if ((initParameters != null) && (initParameters.size() > 0)) {
			return DataStructureL2Helper.toEnumeration(initParameters.keySet());
		}
	...
	public Map<String, Object> getInitParameters() {
		return DataStructureL2Helper.copyMap(initParameters, new HashMap<String, Object>());
	}

	/**
	 * Removes one or multiple initialization parameter.
	 * 
	 * @param name Name of the initialization parameter. If <code>null</code> then
	 *             every initialization parameter is removed.<br>
	 *             <br>
	 */
	...
	public void setProvider(final String name, final Class<? extends AbstractProvider> type,
		...
		if (providers == null) {
			providers = new HashMap<String, com.warework.core.model.Provider>();
		}
	 ...
	 *               <br>
	 */
	public void setProvider(final com.warework.core.model.Provider config) {
		...
		provider.setClazz(config.getClazz());
		provider.setParameters(config.getParameters());

		// Register the Provider.
		providers.put(provider.getName().toString(), provider);
	 ...
	 *         <br>
	 */
	public com.warework.core.model.Provider getProvider(final String name) {
		if ((providers != null) && (providers.containsKey(name))) {
			return (com.warework.core.model.Provider) providers.get(name);
		} else {
			return null;
		}
	}
	...
	public Enumeration<String> getProviderNames() {
		if ((providers != null) && (providers.size() > 0)) {
			return DataStructureL2Helper.toEnumeration(providers.keySet());
		} else {
			return null;
		}
	}
	...
	public void setProviderParameter(final String providerName, final String parameterName,
		...
		final com.warework.core.model.Provider provider = getProvider(providerName);
		...
		if (provider != null) {
			...
			List<com.warework.core.util.bean.Parameter> parameters = provider.getParameters();
			if (parameters == null) {
	...
	public java.lang.Object getProviderParameter(final String providerName, final String parameterName) {
		...
		if (provider != null) {
			...
			// Get the parameters' list.
			final List<com.warework.core.util.bean.Parameter> parameters = provider.getParameters();

			// Remove the parameter only if at least one parameter exists.
			if ((parameters != null) && (parameters.size() > 0)) {

				// Convert the list into a Map.
				final Map<String, Object> params = Parameter.toHashMap(parameters);
	...
	public void removeProviderParameter(final String providerName, final String parameterName) {
		...
		if (provider != null) {
			...
			if (parameters != null) {
				...
				final List<com.warework.core.util.bean.Parameter> parametersCopy = new ArrayList<com.warework.core.util.bean.Parameter>();

				// Copy each parameter that must remain for the Provider.
				for (int i = 0; i < parameters.size(); i++) {

					// Get a parameter.
					final com.warework.core.util.bean.Parameter parameter = parameters.get(i);
	...
	public void setService(final String name, final Class<? extends ServiceFacade> serviceType,
		...
		if (services == null) {
			services = new HashMap<String, com.warework.core.model.Service>();
		}
		...
		// Service.
		// initParams.put(ServiceConstants.PARAMETER_ConfigTarget, service);
	 ...
	 *               <br>
	 */
	public void setService(final com.warework.core.model.Service config) {
		...
		// Setup the Service.
		com.warework.core.model.Service service = null;
		if (config instanceof Serializable) {

			// Get the serializable Service.
			service = config;

			// Set the Scope in the Service.
			service.setScope(this);

		} else if (config instanceof com.warework.core.model.ProxyService) {

			// Get the non-serializable Service.
			final com.warework.core.model.ProxyService proxyService = (com.warework.core.model.ProxyService) config;

			// Create a serializable Service.
			final ProxyService serializableProxyService = new ProxyService();

			// Setup the Service.
			serializableProxyService.setClazz(proxyService.getClazz());
			serializableProxyService.setName(proxyService.getName());
			serializableProxyService.setScope(this);
			serializableProxyService.setParameters(proxyService.getParameters());

			// Get the names of every Client from the non-serializable Service.
			final Enumeration<String> clientNames = proxyService.getClientNames();

			// Copy Clients from non-serializable Service to serializable
			// Service.
			while (clientNames.hasMoreElements()) {

				// Get the name of one Client.
				final String clientName = clientNames.nextElement();

				// Get a non-serializable Client.
				final com.warework.core.model.Client client = (com.warework.core.model.Client) proxyService
						.getClient(clientName);

				// Set the Client.
				serializableProxyService.setClient(client);

			}

			// Set the Service.
			service = serializableProxyService;

		} else if (config instanceof com.warework.core.model.Service) {

			// Create a serializable Service.
			service = new Service();

			// Setup the Service.
			service.setClazz(config.getClazz());
			service.setName(config.getName());
			service.setScope(this);
			service.setParameters(config.getParameters());
		...
		if (service.getParameter(ServiceConstants.PARAMETER_CONFIG_TARGET) == null) {
			service.setParameter(ServiceConstants.PARAMETER_CONFIG_TARGET, service);
		}

		// Register the Service.
		services.put(service.getName().toString(), service);
	 ...
	 *         <br>
	 */
	public com.warework.core.model.Service getService(final String name) {
		if ((services != null) && (services.containsKey(name))) {
			return (com.warework.core.model.Service) services.get(name);
		} else {
			return null;
		}
	}
	...
	public Enumeration<String> getServiceNames() {
		if ((services != null) && (services.size() > 0)) {
			return DataStructureL2Helper.toEnumeration(services.keySet());
		} else {
			return null;
		}
	}
	...
	public void setServiceParameter(final String serviceName, final String parameterName,
		...
		final com.warework.core.model.Service service = getService(serviceName);
		...
		if (service != null) {
			...
			// Get the parameters' list.
			List<com.warework.core.util.bean.Parameter> parameters = service.getParameters();
			if (parameters == null) {
				parameters = new ArrayList<com.warework.core.util.bean.Parameter>();
			}
	...
	public java.lang.Object getServiceParameter(final String serviceName, final String parameterName) {
		...
		if (service != null) {
			...
			// Get the parameters' list.
			final List<com.warework.core.util.bean.Parameter> parameters = service.getParameters();
	 ...
	 *                             <b>WARNING</b>: If Service is registered as a
	 *                             simple Service, it's upgraded to a Proxy Service
	 *                             automatically.<br>
	 *                             <br>
	 ...
	 *         <br>
	 */
	public com.warework.core.model.Client getClient(final String serviceName, final String clientName) {
	...
	public void setClientParameter(final String serviceName, final String clientName, final String parameterName,
		...
		final com.warework.core.model.Client client = getClient(serviceName, clientName);
		...
		if (client != null) {
			...
			// Get the parameters' list.
			List<com.warework.core.util.bean.Parameter> parameters = client.getParameters();
			if (parameters == null) {
				parameters = new ArrayList<com.warework.core.util.bean.Parameter>();
			}
	...
	public java.lang.Object getClientParameter(final String serviceName, final String clientName,
		...
		if (client != null) {
			...
			// Get the parameters' list.
			final List<com.warework.core.util.bean.Parameter> parameters = client.getParameters();
	...
	public void removeClientParameter(final String serviceName, final String clientName, final String parameterName) {
		...
		if (client != null) {
			...
			if (parameters != null) {
				...
				// Create a new List where to copy the parameters that must
				// remain for the Service.
				final List<com.warework.core.util.bean.Parameter> parametersCopy = new ArrayList<com.warework.core.util.bean.Parameter>();
	...
	public void setObjectReference(final String name, final String provider, final String object) {
		...
		if (objectReferences == null) {
			objectReferences = new HashMap<String, com.warework.core.model.ObjectReference>();
		}
		...
		objectReferences.put(name, objectRecord);
	...
	public ObjectReference getObjectReference(final String name) {
		if ((objectReferences != null) && (objectReferences.containsKey(name))) {
			return (ObjectReference) objectReferences.get(name);
		} else {
			return null;
		}
	}
	...
	public Enumeration<String> getObjectReferenceNames() {
		if ((objectReferences != null) && (objectReferences.size() > 0)) {
			return DataStructureL2Helper.toEnumeration(objectReferences.keySet());
		} else {
			return null;
		}
	}
		...
		if (objectReferences != null) {
			if (name != null) {
				...
				if (objectReferences.containsKey(name)) {
					objectReferences.remove(name);
				}

				// Reset the context if required.
				if ((objectReferences != null) && (objectReferences.size() < 1)) {
	 ...
	 *              <br>
	 */
	public void update(final com.warework.core.model.Scope scope) {
	 ...
	 *              <br>
	 */
	private void copyInitParams(final com.warework.core.model.Scope scope) {
	 ...
	 *              <br>
	 */
	private void copyProviders(final com.warework.core.model.Scope scope) {
		...
		if (providerNames != null) {
			while (providerNames.hasMoreElements()) {
				...
				final com.warework.core.model.Provider provider = scope.getProvider(providerName);
	 ...
	 *              <br>
	 */
	private void copyServices(final com.warework.core.model.Scope scope) {
		...
		if (serviceNames != null) {
			while (serviceNames.hasMoreElements()) {
				...
				final com.warework.core.model.Service service = scope.getService(serviceName);
	 ...
	 *              <br>
	 */
	private void copyObjectReferences(final com.warework.core.model.Scope scope) {
		...
		if (objectReferenceNames != null) {
			while (objectReferenceNames.hasMoreElements()) {
				...
				final com.warework.core.model.ObjectReference objectReference = scope
						.getObjectReference(objectReferenceName);
	 ...
	 *              <br>
	 */
	private void copyContextScopes(final com.warework.core.model.Scope scope) {
		...
		if (contextScopeNames != null) {
			while (contextScopeNames.hasMoreElements()) {
				...
				final String contextScopeName = contextScopeNames.nextElement();

				// Set the context Scope in this Scope.
				setContextScope(scope.getContextScope(contextScopeName));
