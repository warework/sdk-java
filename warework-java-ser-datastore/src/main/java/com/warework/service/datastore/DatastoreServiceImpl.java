...
import com.warework.core.service.OperationHandler;
...
/**
 * Implementation of the Service that performs Data Store operations.<br>
 * <br>
 ...
 * places. First, you can find basic and common operations at
 * <a href="http://goo.gl/8dJwTm">DatastoreServiceFacade</a> interface. These
 * are operations that almost every Data Store can execute like connect,
 * disconnect, query, update and commit. Like every Proxy Service, you have to
 * provide the name of the Data Store where to perform any of these
 * operations.<br>
 * <br>
 ...
 * View</a> (specific for relational databases) on top of the JDBC Data Store.
 * <br>
 * <br>
 * <br>
 ...
 * DatastoreServiceFacade interface. We can group these operations as
 * follows:<br>
 * <br>
 * <ul>
 ...
 * queries from files and run them in the Data Store, for example: you can
 * <a href="http://goo.gl/HMZJrt">setup a Provider to read SQL statements</a>
 * from text files and execute each one in a relational database, just by
 * specifying the name of the Provider and the name of the text file to read.
 * For the next sample code we are going to suppose that:<br>
 * <br>
 ...
 * data provided by another View of the stack or directly from the Data
 * Store.<br>
 * <br>
 * <img alt="" src=
 ...
 * <br>
 * <br>
 * You can also configure Data Stores with initialization parameters like
 * this:<br>
 * <br>
 * <br>
 ...
 * @version 2.0.1
 */
public final class DatastoreServiceImpl extends AbstractService implements DatastoreServiceFacade {

	// ///////////////////////////////////////////////////////////////////
	// CONSTANTS
	// ///////////////////////////////////////////////////////////////////

	// Class name for the Data Store Extension operation handler.
	private static final String DATASTORE_EXTENSION_OPERATION_HANDLER_CLASS = ScopeL1Constants.PACKAGE_SERVICE
			+ ReflectionL1Helper.PACKAGE_SEPARATOR + DatastoreServiceConstants.SERVICE
			+ ReflectionL1Helper.PACKAGE_SEPARATOR
			+ StringL1Helper.firstLetterToUpperCase(DatastoreServiceConstants.SERVICE) + "ExtensionOperationHandler";
	...
	// Context for clients.
	private Map<String, AbstractViewsLogic> clients;
	...
	/**
	 * Removes every Data Store that exist in this Proxy Service.
	 * 
	...
	/**
	 * Creates a Data Store and binds it to this Service, using the name specified.
	 * 
	 * @param datastoreName        The name to which the Data Store is bound in the
	 *                             Service. If a Data Store of the same name is
	 *                             already bound to this Service, an exception is
	 *                             thrown.<br>
	 *                             <br>
	 * @param connectorType        Implementation of the Connector that specifies
	 *                             which data store to create and how to create the
	 ...
	 *                          <br>
	 */
	public void createClient(String datastoreName, Class<? extends ConnectorFacade> connectorType,
			Map<String, Object> connectionParameters) throws ServiceException {
		...
		try {
			...
			connector = (AbstractConnector) connectorType.newInstance();

			// Initialize the Connector.
			connector.init(this, datastoreName, connectionParameters);

		} catch (Exception e) {
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot create Data Store '" + datastoreName + "' in Service '" + getName()
							+ "' because the instantiation of the Connector generated the following error: "
							+ e.getMessage(),
					e, LogServiceConstants.LOG_LEVEL_WARN);
		}

		// Validate that no Data Store already exists with this name.
		if ((clients != null) && (clients.containsKey(datastoreName))) {
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot create Data Store '" + datastoreName + "' in Service '" + getName()
							+ "' as a Data Store with this name already exists.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}

		// Create and initialize the Data Store.
		AbstractViewsLogic datastoreView = null;
		try {
			datastoreView = new DefaultView(connector);
		} catch (ClientException e) {
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot create Data Store '" + datastoreName + "' in Service '" + getName()
							+ "' because the initialization of the Data Store generated the following error: "
							+ e.getMessage(),
					e, LogServiceConstants.LOG_LEVEL_WARN);
		}
		...
		if (clients == null) {
			clients = new HashMap<String, AbstractViewsLogic>();
		}

		// Save this Data Store.
		clients.put(datastoreName, datastoreView);

		// Log a message.
		getScopeFacade().log(
				"WAREWORK created Data Store '" + connector.getClientName() + "' in Service '" + getName() + "'.",
		...
		if ((connectionParameters != null)
			...
			Object connectOnCreate = connectionParameters.get(AbstractConnector.PARAMETER_CONNECT_ON_CREATE);
		...
		// Connect the Data Store when required.
		if (connectClient) {
			connect(datastoreName);
		}
	...
	public Enumeration<String> getClientNames() {
		...
		// Return Data Store names if they exist.
	...
	/**
	 * Validates if a Data Store is bound to this Service.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @return <code>true</code> if the Data Store exists and <code>false</code> if
	 *         Data Store does not exists.<br>
	 *         <br>
	 */
	public boolean existsClient(String datastoreName) {

		// Search for the client.
		if ((clients != null) && (datastoreName != null) && (!datastoreName.equals(""))) {
			return clients.containsKey(datastoreName);
		}
	...
	/**
	 * Removes a Data Store bound to this Service. If this Service does not have a
	 * Data Store bound with the specified name, this method does nothing. If Data
	 * Store is connected this method disconnects it first.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 ...
	 *                          <br>
	 */
	public void removeClient(String datastoreName) throws ServiceException {

		// Get the client.
		AbstractViewsLogic client = (AbstractViewsLogic) getDatastore(datastoreName);

		// Remove the client.
		if (client != null) {

			// Disconnect the Data Store first.
			if (client.isConnected()) {
				try {
					client.disconnect();
				} catch (ClientException e) {
					throw new ServiceException(getScopeFacade(),
							"WAREWORK cannot remove Data Store '" + datastoreName + "' in Service '" + getName()
									+ "' because Data Store cannot be disconnected.",
							e, LogServiceConstants.LOG_LEVEL_WARN);
				}
			}

			// Remove the data source.
			clients.remove(datastoreName);
			...
			// Log a message.
			getScopeFacade().log("WAREWORK removed Data Store '" + datastoreName + "' in Service '" + getName() + "'.",
					LogServiceConstants.LOG_LEVEL_INFO);

		} else {
			getScopeFacade().log("WAREWORK cannot remove Data Store '" + datastoreName + "' in Service '" + getName()
					+ "' because Data Store does not exists.", LogServiceConstants.LOG_LEVEL_WARN);
		}

	}

	/**
	 * Removes all the Data Stores bound to this Service.
	 * 
	 * @throws ServiceException If there is an error when trying to remove the
	 *                          Clients.<br>
	 *                          <br>
	...
	public void removeAllClients() throws ServiceException {
		...
		Enumeration<String> clientsNames = getClientNames();
		...
		if (clientsNames != null) {
			while (clientsNames.hasMoreElements()) {
				...
				String clientName = clientsNames.nextElement();
	...
	/**
	 * Gets the class that implements the Data Store.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @return Type of the Data Store or <code>null</code> if Data Store does not
	 *         exists.<br>
	 *         <br>
	 */
	public Class<? extends ClientFacade> getClientType(String datastoreName) {
		...
		if (client != null) {
			return client.getDatastore().getConnector().getClientType();
		} else {
			return null;
		}
	...
	/**
	 * Validates if a Data Store is the same type of a given class.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @param datastoreType Class to compare with the class of the Data Store.<br>
	 *                      <br>
	 * @return <code>true</code> if the Data Store's type is the same as given class
	 *         and <code>false</code> if not.<br>
	 *         <br>
	 */
	public boolean isClientType(String datastoreName, Class<? extends ClientFacade> datastoreType) {

		// Get the type of the client.
		Class<? extends ClientFacade> type = getClientType(datastoreName);

		// Return if both are the same.
		return ((type != null) && (type.equals(datastoreType)));
	...
	/**
	 * Gets the connection of a Data Store. First you'll need to connect the Data
	 * Store to retrieve the connection, otherwise this method will return null.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 ...
	 *         <br>
	 */
	public Object getConnection(String datastoreName) {

		// Get the data source.
		AbstractViewsLogic client = getDatastore(datastoreName);
		...
		if (client != null) {
			...
			// Log a message.
			getScopeFacade().log("WAREWORK cannot get the connection for Data Store '" + datastoreName
					+ "' in Service '" + getName() + "' because Data Store does not exists.",
	...
	/**
	 * Opens a connection with a Data Store. If Data Store's connection is already
	 * open this method throws an exception.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 ...
	 *                          <br>
	 */
	public void connect(String datastoreName) throws ServiceException {
		...
		if (client != null) {
			try {
				...
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot connect Data Store '" + datastoreName + "' at Service '" + getName() + "'.", e,
						LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
			throw new ServiceException(
					getScopeFacade(), "WAREWORK cannot connect Data Store '" + datastoreName + "' in Service '"
							+ getName() + "' because Data Store does not exists.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	...
	/**
	 * Closes a connection with a Data Store.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 ...
	 *                          <br>
	 */
	public void disconnect(String datastoreName) throws ServiceException {
		...
		if (client != null) {
			try {
				...
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot disconnect Data Store '" + datastoreName + "' at Service '" + getName() + "'.",
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
			getScopeFacade().log("WAREWORK cannot disconnect Data Store '" + datastoreName + "' in Service '"
					+ getName() + "' because Data Store does not exists.", LogServiceConstants.LOG_LEVEL_WARN);
		}

	}

	/**
	 * Closes the connection of every Data Store.
	 * 
	 * @throws ServiceException If there is an error when trying to disconnect the
	 *                          Clients.<br>
	 *                          <br>
	...
	public void disconnectAll() throws ServiceException {
		...
		Enumeration<String> clientNames = getClientNames();
		...
		if (clientNames != null) {
			while (clientNames.hasMoreElements()) {
				...
				// Get the name of a client.
				String clientName = (String) clientNames.nextElement();
	...
	/**
	 * Validates if a Data Store is connected.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @return <code>true</code> if the Data Store is connected and
	 *         <code>false</code> if the Data Store is not connected.<br>
	 *         <br>
	 */
	public boolean isConnected(String datastoreName) {

		// Get the data source.
		AbstractViewsLogic client = getDatastore(datastoreName);

		// Decide if the Data Store is connected.
		if (client != null) {
	 ...
	 *                      Service.<br>
	 *                      <br>
	 * @param viewName      View of the Data Store where to execute the query. Use
	 *                      <code>null</code> to execute the query in the Current
	 *                      View of the Data Store.<br>
	 *                      <br>
	 * @param statement     Specific query language accepted by the Data Store (SQL,
	 *                      for example).<br>
	 *                      <br>
	 * @return Object that holds the result of the query.<br>
	 *         <br>
	 * @throws ServiceException If there is an error when trying to query the Data
	 *                          Store.<br>
	 *                          <br>
	 */
	public Object query(String datastoreName, String viewName, Object statement) throws ServiceException {

		// Get the view of the Data Store.
		AbstractViewsLogic view = null;
		if (viewName == null) {
			view = getDatastore(datastoreName);
		} else {
			view = getView(datastoreName, viewName);
		}

		// Perform the query in the data source.
		if (view != null) {
			try {
				return view.query(statement, null);
			} catch (ClientException e) {
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot execute the query in Data Store '" + datastoreName + "' at Service '"
								+ getName() + "' because the Data Store reported the following error: "
								+ e.getMessage(),
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
			throw new ServiceException(
					getScopeFacade(), "WAREWORK cannot execute the query in Data Store '" + datastoreName
							+ "' at Service '" + getName() + "' because Data Store does not exists.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	 ...
	 *                      View of the Data Store.<br>
	 *                      <br>
	 * @param providerName  Name of the Provider where to retrieve a query
	 *                      object.<br>
	 *                      <br>
	 * @param statementName Name of the query to retrieve from the Provider.<br>
	 *                      <br>
	 * @param values        Map where the keys are the names of the variables in the
	 *                      query-string loaded and the values those that will
	 *                      replace the variables. Each Data Store may process this
	 *                      values in a specific way (not every Data Store support
	 *                      the replacement of variables). When the query statement
	 *                      loaded from the Provider represents a String object,
	 *                      every variable must be inside '${' and '}' so the
	 *                      variable CAR must be in this query-string as '${CAR}'.
	 ...
	 *                          <br>
	 */
	public Object query(String datastoreName, String viewName, String providerName, String statementName,
			Map<String, Object> values) throws ServiceException {

		// Get the query.
		Object query = getScopeFacade().getObject(providerName, statementName);

		// Execute the query only if it exists.
		if (query != null) {
			...
			// Perform the query in the Data Store.
			if (view != null) {
				try {
					return view.query(query, values);
				} catch (ClientException e) {
					throw new ServiceException(getScopeFacade(),
							"WAREWORK cannot execute the query in Data Store '" + datastoreName + "' at Service '"
									+ getName() + "' because the Data Store reported the following error: "
			...
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot read a query object from Provider '" + providerName + "' in Data Store '"
							+ datastoreName + "' at Service '" + getName()
							+ "' because Provider does not have query/object '" + statementName + "'.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	 ...
	 *                      Service.<br>
	 *                      <br>
	 * @param viewName      View of the Data Store where to execute the statement.
	 *                      Use <code>null</code> to execute the statement in the
	 *                      Current View of the Data Store.<br>
	 *                      <br>
	 * @param statement     Indicates the specific operation to perform and the data
	 *                      to update in the Data Store.<br>
	 *                      <br>
	 * @throws ServiceException If there is an error when trying to update the Data
	 *                          Store.<br>
	 *                          <br>
	 */
	public void update(String datastoreName, String viewName, Object statement) throws ServiceException {
		...
		if (view != null) {
			try {
				view.update(statement, null);
			} catch (ClientException e) {
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot execute the update operation in Data Store '" + datastoreName
								+ "' at Service '" + getName()
								+ "' because the Data Store reported the following error: " + e.getMessage(),
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot execute the update operation in Data Store '" + datastoreName + "' at Service '"
							+ getName() + "' because Data Store does not exists.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	 ...
	 *                      Current View of the Data Store.<br>
	 *                      <br>
	 * @param providerName  Name of the Provider where to retrieve a statement.<br>
	 *                      <br>
	 * @param statementName Name of the statement to retrieve from the Provider.<br>
	 *                      <br>
	 * @param values        Map where the keys are the names of the variables in the
	 *                      query-string loaded and the values those that will
	 *                      replace the variables. Each Data Store may process this
	 ...
	 *                          <br>
	 */
	public void update(String datastoreName, String viewName, String providerName, String statementName,
			Map<String, Object> values) throws ServiceException {

		// Get the statement.
		Object statement = getScopeFacade().getObject(providerName, statementName);

		// Execute the statement only if it exists.
		if (statement != null) {
			...
			// Execute the statement in the Data Store.
			if (view != null) {
				try {
					view.update(statement, values);
				} catch (ClientException e) {
					throw new ServiceException(getScopeFacade(),
							"WAREWORK cannot execute the update operation in Data Store '" + datastoreName
									+ "' at Service '" + getName()
			...
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot read a statement from Provider '" + providerName + "' in Data Store '"
							+ datastoreName + "' at Service '" + getName() + "' because Provider does not have object '"
							+ statementName + "'.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	...
	/**
	 * Makes in a Data Store all changes made since the previous commit permanent.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @throws ServiceException If there is an error when trying to commit the Data
	 *                          Store.<br>
	 *                          <br>
	 */
	public void commit(String datastoreName) throws ServiceException {
		...
		if (client != null) {
			try {
				...
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot commit Data Store '" + datastoreName + "' at Service '" + getName()
								+ "' because the Data Store reported the following error: " + e.getMessage(),
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
	 ...
	 *                          <br>
	 */
	public void commitAll() throws ServiceException {

		// Get the IDs of the Data Stores.
		Enumeration<String> clientNames = getClientNames();

		// Commit each Data Store.
		if (clientNames != null) {
			while (clientNames.hasMoreElements()) {
	 ...
	 *                      Service.<br>
	 *                      <br>
	 * @param viewType      Implementation of the view. Must extends the
	 *                      <code>com.warework.service.datastore.AbstractDatastoreView</code>
	 *                      class.<br>
	 *                      <br>
	 * @param viewName      Name for the View to include in the Data Store.<br>
	 *                      <br>
	 * @param providerName  Name of the default Provider where to retrieve
	 *                      statements for the View. This parameter can be
	 *                      <code>null</code>.<br>
	 *                      <br>
	 * @param parameters    Initialization parameters (as string-object pairs) for
	 *                      the View.<br>
	 *                      <br>
	 * @throws ServiceException If there is an error when trying to add the View in
	 *                          the Data Store.<br>
	 *                          <br>
	 */
	public void addView(String datastoreName, Class<? extends DatastoreView> viewType, String viewName,
			String providerName, Map<String, Object> parameters) throws ServiceException {

		// Validate the name of the View.
		if ((viewName == null) || (viewName.equals(CommonValueL1Constants.STRING_EMPTY))) {
			throw new ServiceException(getScopeFacade(),
					"WAREWORK cannot add the View in Data Store '" + datastoreName + "' of Service '" + getName()
							+ "' because the name for the View is null or empty.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		} else {

			// Get the Data Store.
			AbstractViewsLogic datastore = (AbstractViewsLogic) getDatastore(datastoreName);

			// Validate that no Views in the stack have the same name as the
			// given one.
			if (datastore != null) {
				while (datastore != null) {
					if (datastore.getViewName().equals(viewName)) {
						throw new ServiceException(getScopeFacade(),
								"WAREWORK cannot add the View in Data Store '" + datastoreName + "' of Service '"
										+ getName() + "' because there is another View named '" + viewName + "'.",
								null, LogServiceConstants.LOG_LEVEL_WARN);
					} else {

						// Get the parent view.
						AbstractViewsLogic view = datastore.getParentView();

						// Validate the view.
						if (view instanceof DefaultView) {
							break;
						} else {
							datastore = (AbstractViewsLogic) view;
						}
		...
		// Get the Data Store.
		AbstractViewsLogic datastore = getDatastore(datastoreName);

		// Validate that Data Store exists.
		if (datastore != null) {

			// Create an instance of the view.
			AbstractDatastoreView view = null;
			try {
				view = (AbstractDatastoreView) viewType.newInstance();
			} catch (Exception e) {
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot add the View in Data Store '" + datastoreName + "' of Service '" + getName()
								+ "' because the View is null or does not extends the '"
								+ AbstractDatastoreView.class.getName() + "' class.",
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}

			// Initialize the View.
			try {
				view.init(datastore, viewName, providerName, parameters);
			} catch (ClientException e) {
				throw new ServiceException(getScopeFacade(),
						"WAREWORK cannot add the View in Data Store '" + datastoreName + "' of Service '" + getName()
								+ "' because the initialization of the View generated the following error: "
								+ e.getMessage(),
						e, LogServiceConstants.LOG_LEVEL_WARN);
			}

			// Update the view.
			clients.put(datastoreName, view);

		} else {
			throw new ServiceException(
					getScopeFacade(), "WAREWORK cannot add a view in Data Store '" + datastoreName + "' of Service '"
							+ getName() + "' because Data Store does not exists.",
					null, LogServiceConstants.LOG_LEVEL_WARN);
		}
	...
	public void removeView(String datastoreName) {
		...
		// Remove the view from the Data Store.
		if (datastore != null) {
			if (datastore.getParentView() != null) {
				clients.put(datastoreName, datastore.getParentView());
			} else {
				getScopeFacade().log("WAREWORK cannot remove a view from Data Store '" + datastoreName
						+ "' in Service '" + getName() + "' because Data Store does not have more Views.",
						LogServiceConstants.LOG_LEVEL_WARN);
			}
		} else {
			getScopeFacade().log("WAREWORK cannot remove a view from Data Store '" + datastoreName + "' in Service '"
					+ getName() + "' because Data Store does not exists.", LogServiceConstants.LOG_LEVEL_WARN);
		}
	 ...
	 *                      <br>
	 */
	public void removeAllViews(String datastoreName) {
		while (!isDefaultView(datastoreName)) {
			removeView(datastoreName);
		}
	}
	 ...
	 *         Store and <code>false</code> if not or no Data Store is bound with
	 *         the specified name.<br>
	 *         <br>
	 */
	public boolean isDefaultView(String datastoreName) {

		// Get the Data Store.
		AbstractViewsLogic datastore = getDatastore(datastoreName);

		// Remove the view.
		if ((datastore != null) && (datastore.getParentView() == null)) {
			return true;
		}
	 ...
	 *         <br>
	 */
	public AbstractDatastoreView getView(String datastoreName) {
		return getView(datastoreName, (Class<? extends DatastoreView>) null);
	}
	 ...
	 *                      Service.<br>
	 *                      <br>
	 * @param viewType      Type of the View that determines up to where to search.
	 *                      Use <code>null</code> to get the Current View of a Data
	 *                      Store.<br>
	 *                      <br>
	 * @return View of the Data Store.<br>
	 *         <br>
	 */
	public AbstractDatastoreView getView(String datastoreName, Class<? extends DatastoreView> viewType) {

		// Retrieve the view of the Data Store.
		if (viewType == null) {

			// Get the Data Store.
			AbstractViewsLogic datastore = getDatastore(datastoreName);

			// Return the Data Store view.
			if (datastore != null) {
				if (!(datastore instanceof DefaultView)) {
					return (AbstractDatastoreView) datastore;
				} else {
					getScopeFacade().log(
							"WAREWORK cannot get the view of Data Store '" + datastoreName + "' in Service '"
									+ getName() + "' because Data Store does not have Views.",
							LogServiceConstants.LOG_LEVEL_WARN);
				}
			} else {
				getScopeFacade().log("WAREWORK cannot get the view of Data Store '" + datastoreName + "' in Service '"
						+ getName() + "' because Data Store does not exists.", LogServiceConstants.LOG_LEVEL_WARN);
			}
			...
			// Get the Data Store.
			AbstractDatastoreView datastore = (AbstractDatastoreView) getDatastore(datastoreName);

			// Search for the view.
			if (datastore != null) {
				while (datastore != null) {
					if (datastore.getClass().equals(viewType)) {
						return datastore;
					} else {
						...
						if (view instanceof DefaultView) {
							...
							// Log a message.
							getScopeFacade().log("WAREWORK cannot get the View of Data Store '" + datastoreName
									+ "' in Service '" + getName()
									+ "' because given type of the View does not exists in the stack of Views of the Data Store.",
									LogServiceConstants.LOG_LEVEL_WARN);
							...
							datastore = (AbstractDatastoreView) view;
						}
	 ...
	 *                      Service.<br>
	 *                      <br>
	 * @param viewName      Name of the View.<br>
	 *                      <br>
	 * @return View of the Data Store.<br>
	 *         <br>
	 */
	public AbstractDatastoreView getView(String datastoreName, String viewName) {
		...
		if (datastore != null) {
			...
			// Validate that default View name is not given.
			if (viewName.equals(DefaultView.DEFAULT_VIEW_NAME)) {

				// Log a message.
				getScopeFacade().log(
						"WAREWORK cannot get View '" + viewName + "' from Data Store '" + datastoreName
								+ "' in Service '" + getName()
								+ "' because no View exists with this name in the stack of Views of the Data Store.",
						LogServiceConstants.LOG_LEVEL_WARN);
			...
			while (datastore != null) {
				if (datastore.getViewName().equals(viewName)) {
					...
					if (view instanceof DefaultView) {
						...
						// Log a message.
						getScopeFacade().log("WAREWORK cannot get View '" + viewName + "' from Data Store '"
								+ datastoreName + "' in Service '" + getName()
								+ "' because no View exists with this name in the stack of Views of the Data Store.",
								LogServiceConstants.LOG_LEVEL_WARN);
	 ...
	 *         <br>
	 */
	public Enumeration<String> getViewNames(String datastoreName) {

		// Get the Data Store.
		AbstractViewsLogic view = (AbstractViewsLogic) getDatastore(datastoreName);

		// Get the name of every View of the Data Store.
		if (view != null) {

			// Store the name of the Views in a list.
			List<String> viewNames = new ArrayList<String>();

			// Get the name of every View.
			...
			// Return the names of the Views.
			return DataStructureL1Helper.toEnumeration(viewNames);

		} else {
			getScopeFacade().log("WAREWORK cannot get the names of the Views of Data Store '" + datastoreName
					+ "' in Service '" + getName() + "' because Data Store does not exists.",
					LogServiceConstants.LOG_LEVEL_WARN);
		}
	 ...
	 * 
	 * @param serviceConfig Service's configuration.<br>
	 *                      <br>
	...
	protected void configure(Object serviceConfig) throws ServiceException {
		if (serviceConfig instanceof DatastoreService) {

			// Get the configuration of the Data Store Service.
			DatastoreService config = (DatastoreService) serviceConfig;

			// Configure the Data Stores.
			Enumeration<String> datastoreNames = config.getClientNames();
			if (datastoreNames != null) {
				try {
					while (datastoreNames.hasMoreElements()) {

						// Get the name of the Data Store.
						String datastoreName = datastoreNames.nextElement();

						// Get the configuration of a Data Store.
						Datastore datastore = (Datastore) config.getClient(datastoreName);

						// Get the type of the Connector for the Data Store.
						Class<? extends ConnectorFacade> connectorType = ReflectionL1Helper.getConnectorType(this,
								datastore.getConnector());

						// Get the configuration parameters for the Connector.
						Map<String, Object> datastoreParams = Parameter.toHashMap(datastore.getParameters());

						// Create the Data Store.
						createClient(datastoreName, connectorType, datastoreParams);
						...
						if ((views != null) && (views.size() > 0)) {
							for (int i = 0; i < views.size(); i++) {
								...
								// Get the type of the Connector for the data
								// store.
								Class<? extends DatastoreView> viewType = getViewType(view.getClazz());

								// Get the configuration parameters for the
								// Connector.
								Map<String, Object> viewParams = Parameter.toHashMap(view.getParameters());

								// Add the view.
								addView(datastoreName, viewType, view.getName(), view.getStatementProvider(),
										viewParams);
					...
					throw new ServiceException(getScopeFacade(),
							"WAREWORK cannot configure Service '" + getName()
									+ "' because the class of a Connector for a Data Store is not found.",
							e, LogServiceConstants.LOG_LEVEL_WARN);
				}
			}

		} else if (serviceConfig instanceof ProxyService) {

			// Get the Data Store manager configuration.
			ProxyService proxyServiceConfig = (ProxyService) serviceConfig;

			// Get clients.
			Enumeration<String> clientNames = proxyServiceConfig.getClientNames();
			...
			if (clientNames != null) {
				try {
					while (clientNames.hasMoreElements()) {
						...
						Client client = proxyServiceConfig.getClient(clientName);

						// Get the type of the Connector.
						Class<? extends ConnectorFacade> connectorType = (Class<? extends ConnectorFacade>) Class
								.forName(client.getConnector());
	...
	/**
	 * Initializes the Data Store Service.
	 * 
	...
	protected void initialize() throws ServiceException {
		...
		// Create the Data Store Extension operation handler if it is available.
		// If not, create the default Data Store operation handler.
		try {

			// Get the class for the Data Store Extension operation handler.
			Class<? extends OperationHandler> operationHandlerType = (Class<? extends OperationHandler>) Class
					.forName(DATASTORE_EXTENSION_OPERATION_HANDLER_CLASS);

			// Create a new instance of the operation handler.
			operationHandler = (OperationHandler) operationHandlerType.newInstance();

		} catch (Exception e) {
			operationHandler = new DatastoreOperationHandler();
		}
	...
	/**
	 * Gets a Data Store. It is the View on top of the stack of Views of the Data
	 * Store.
	 * 
	 * @param datastoreName The name to which the Data Store is bound in the
	 *                      Service.<br>
	 *                      <br>
	 * @return A Data Store or <code>null</code> if it does not exists.<br>
	 *         <br>
	 */
	protected AbstractViewsLogic getDatastore(String datastoreName) {

		// Return the client.
		if ((datastoreName != null) && (!datastoreName.equals(CommonValueL1Constants.STRING_EMPTY)) && (clients != null)
				&& (clients.containsKey(datastoreName))) {
			return (AbstractViewsLogic) clients.get(datastoreName);
		}
	...
	/**
	 * Gets the class of a View.
	 * 
	 * @param className Name of the class. It can be a short name, for example: JPA.
	 *                  If so, this method automatically builds the full name for
	 *                  the class:
	 *                  <code>com.warework.service.datastore.client.JPAViewImpl</code>
	 *                  .<br>
	 *                  <br>
	 * @return Service class.<br>
	 *         <br>
	 * @throws ClassNotFoundException If there is an error when trying to retrieve
	 *                                the View type.<br>
	 *                                <br>
	 */
	@SuppressWarnings("unchecked")
	private Class<? extends DatastoreView> getViewType(String className) throws ClassNotFoundException {

		// Update the class name when it is not complete.
		if (className.indexOf(ReflectionL1Helper.PACKAGE_SEPARATOR) < 0) {
			className = ScopeL1Constants.PACKAGE_SERVICE + ReflectionL1Helper.PACKAGE_SEPARATOR
					+ DatastoreServiceConstants.SERVICE + ReflectionL1Helper.PACKAGE_SEPARATOR
					+ CommonValueL1Constants.STRING_CLIENT + ReflectionL1Helper.PACKAGE_SEPARATOR
					+ StringL1Helper.firstLetterToUpperCase(className)
					+ StringL1Helper.firstLetterToUpperCase(CommonValueL1Constants.STRING_VIEW)
					+ StringL1Helper.firstLetterToUpperCase(CommonValueL1Constants.STRING_IMPL);
		}

		// Return component type.
		return (Class<? extends DatastoreView>) Class.forName(className);
